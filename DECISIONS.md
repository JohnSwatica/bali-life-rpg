...
This amends GATE v2's freeze: the ban was against big-bang speculative map builds, and that ban stands — what changes is that small reviewable increments are now authorized (and expected) per-packet. The GDD's six-district vision is reached by drip, not by dump. First application: RPG-20260708-06 (still unexecuted) was retrofitted with the first increment — a walkable paddy-edge path near The Corner that lets the player physically approach the yellowing-paddy mystery seed; RPG-20260708-07 carries an explicit `MAP DELTA: none` as the infra-packet example. The `MAP DELTA:` line is an RPG-project extension of the global packet format, not a change to the cross-project standard.
