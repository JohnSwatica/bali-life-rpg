...
## 2026-07-12 - Corrected A Wrong Sequencing Instruction; Codex's Block Was The System Working

Codex reported RPG-20260712-03 blocked, citing `origin/main` not having RPG-20260712-02 merged and 26 files of uncommitted work in the tree. Investigation found the real situation: RPG-20260712-02 (cinematic cold-open) and RPG-20260712-04 (Warung Rush) had both been fully implemented — 259 tests passing, build green, no stub/TODO markers, careful schema-safe persistence handling — but neither had been committed before the other started, so they sat as one mixed uncommitted diff across 30 files including several files both packets touched.

Codex was correct to refuse to proceed and its reasoning was sound in spirit (03's checkpoint-tap-beat sweep needs 02's new delivery to exist in its branch) but wrong in the specific remedy it was pointed at: this project has never merged individual packets to `main` mid-stream — every packet lands as a commit on the shared branch `feat/rpg-20260706-09-rio-race`, with one open PR (#1) for the whole stack. That was `docs/prompts/README.md`'s own error: the RPG-20260712-03 sequencing note told Codex to "branch from `main` only after 02 is merged," which was never going to happen and would have stalled the queue indefinitely had it not been caught.

Fix applied: committed the combined 02+04 work as one dual-tagged commit (`f84bfda`, splitting was rejected as too risky given shared-file entanglement — see that commit's message for the full breakdown, including 04's known gap: its logic is tested but the in-scene screenshot/self-assessment proof its own DoD required was never produced). Corrected the README's standing rules to state actual practice plainly (one commit per packet on the shared branch, no per-packet PR/merge, John merges the stack as a unit) and added an explicit rule: packets authorized to "run in parallel" describe non-colliding LOGIC, not permission to skip committing between them — commit the first packet's result before starting the next, even when nothing was expected to conflict. RPG-20260712-03 is now correctly unblocked to branch from the current commit on the existing branch, not from `main`.

## 2026-07-13 - Live Playtest Finds A Hard Act-0 Blocker; Story Reskin Was Never Story Content

John played the merged RPG-20260712 build and reported being stuck at "Get onto NusaDrop" with "nothing to interact with" and "signs not making sense." Live headless repro (Claude, real keypresses, screenshots) confirmed a genuine hard blocker rather than a misunderstanding: the Act 0 `buy_meal_and_coffee` objective targets `milk_madu_berawa`'s authored coordinate, but the nearest interactable venue at that exact point resolves to a completely different venue, `milu_by_nook` ("Milu by Nook" — a separate curated real-world catalog entry, distinct id, no relation to the gameplay venue "Milk & Madu"). The interaction prompt and opened activity panel both say "Milu by Nook," which does not match the quest text naming "Milk & Madu" — so a player reading the sign at face value reasonably concludes they're in the wrong place and never presses E there. It happens to be survivable by luck (the wrong venue's generic cafe activities technically satisfy the step's completion check), but no real player would discover that from the UI as presented. This fully blocks Act 0 completion for every new player, which is the worst possible severity right now — it stands directly between the (well-received) new cinematic opening and everything downstream. `RPG-20260713-01` packets the fix, including an audit for sibling collisions elsewhere in the authored street (the curated-venue coordinate set is known to contain many `estimatedCoord`/fallback entries per `data/osm/berawa.curated-coords.json`, so this is plausibly not a one-off).

Separately, John pointed out — correctly — that RPG-20260712-01 (the "v4 canon swap" packet) only reskinned NAMES onto existing generic systems (Rio's dialogue lines became Leo's, "Jalan" became "NusaDrop," etc.) and never implemented any of STORY_BIBLE.md v4's actual STORY BEATS (the rate-cut inciting hook, Leo's escalating rivalry cadence, Made's room offer, the midpoint breakdown, Julian Vance's pressure campaign, the moral choice points beyond the opening one). That gap is real and is exactly why post-intro Act 1 still reads as generic walk/press-E fetch content despite the narrative pivot. `RPG-20260713-02` packets the first genuine story beat — v4's Act 1 inciting hook (NusaDrop's rate cut + Leo mocking the player at a pickup hub), scoped deliberately to one felt beat rather than the whole Act 1 backbone at once, with the remaining turning points named as explicit follow-ups so the gap doesn't quietly reopen.

Process note: RPG-20260712-03 landed as its own branch (`feat/rpg-20260712-03-steering-delivery`) rather than continuing directly on the shared integration branch, which briefly stalled the dev server / working-tree state understanding this session. It was a clean fast-forward and has been merged into `feat/rpg-20260706-09-rio-race` and pushed; no data was lost.

## 2026-07-13 - Day-1 Sensation Is Transient Control Over Existing World Systems

RPG-20260713-03 adds no parallel clock, lighting engine, persisted weather, or story state. Authored Day-1 time is a guarded beat control that writes the existing `world.clock.minuteOfDay` only during unfinished Act 0; normal `advanceClock`, `getTimePhase`, NPC schedules, and `updateLighting` continue from that value. Morning/noon color accents exist only while the authored Day-1 control is active. The globally shared dusk/night tints and existing lantern glows are tuned modestly for the dramatic night ride while preserving street legibility.

Weather has one transient authority: `WorldWeatherController`. Authored scene weather outranks delivery-requested rain; delivery rain can seed the controller only when no scene owns it. The ride model no longer reads a delivery condition string for slickness — it reads canonical weather state, which also drives the visible pooled streaks, wet-street tint, rain ambient bed, and storm thunder/flash. This prevents invisible slick roads and visible-but-dry parallel flags. Weather is intentionally clear after reload, so schema v11 is unchanged.

Ambient sound remains procedural WebAudio and behind the existing unlock/mute setting. Phase, weather, and scene context select one of four beds (`morningStreet`, `cafeInterior`, `rain`, `nightQuiet`); story/scene code may set an explicit scene bed without owning audio nodes. The kos change is dressing only: dim palette, mattress/crate/shelf/damp marks, and a single exposed-bulb radius. No furniture/upgrades system, geometry, story beat, quest content, payout/rate-cut math, or Leo/race change is part of this packet.

## 2026-07-14 - Act-Card Scrim Is The Standard Story-Card Treatment
...
## 2026-07-14 - Phone Opportunity Generation Is An Explicit Story/Goal Allowlist

RPG-20260714-05 applies the five-verb test at generation time, not by deleting the opportunity engine or its data. Only six templates may repopulate the dynamic pool: the No-Questions Package choice and five templates consumed by the current Act 2/Act 3 goal/readiness code. Fourteen standalone receipt/key/deal/rumor/social/errand templates remain defined for legacy-save safety and possible Act 2 restaging, but the maintenance loop never selects them. Revival requires a named scene or playable verb; none may return as a menu-only errand.

Feed priority is now structural rather than timestamp-only: current goal and authored story/status messages, then paying NusaDrop work, then remaining live items/history. Board deliveries, Ibu Sari's catering run, Kadek and Made story offers, Leo/NusaDrop beats, rent status, and the No-Questions choice remain on their existing systems and values.

The public phone navigation is Feed / Map / Goals / Profile. Goals is the player-facing successor to Quests and promotes active incomplete goals such as Made's room; Profile retains rating and reputation. Contacts, Threads, Quests, Calendar, Events, Venues, and Community remain valid internal tabs so future Act 2 surfaces and existing proof scripts compile, but they are absent from the strip. Current participation is not stranded: clubs join at their home venues, events resolve on-site, discovered venues remain on Map, NPC arcs remain world-driven, and reputation remains in Profile. Multiplayer locked chrome is unchanged.

Schema stays v11. Stale cut-template messages remain inert history, while an already-live/accepted cut opportunity from an old save retains its definition and can resolve or expire normally. New instances cannot spawn. The phone shell resynchronizes to camera movement while open so interior exits and viewport changes cannot leave the panel off-screen.

## 2026-07-15 - Authored Downfalls Arm At Acceptance And Resolve Through Existing Fail-Forward Systems

RPG-20260714-07 establishes the authored-downfall pattern for one-time story reversals. A downfall is never placed in a random condition pool. It arms only when its named story prerequisites are complete and the first eligible action is successfully accepted; a persisted one-time flag owns firing, and a persisted active flag owns any interruption-safe sequence until resolution. Runtime presentation may add a deterministic position/time gate, but it may not decide whether the event exists.

The Act 1 transmission reversal uses only v11's existing flag maps plus canonical delivery/rating/scooter/cargo fields. Mounted world-travel time and a fixed distance band trigger the blowout; cargo integrity becomes zero, the existing stuck-bike visual becomes a slow push, and the run is made late but remains completable for its base fare. At handoff the global driver rating is set to the bible's authored 3.2 value, not decremented or averaged. The repair counter clears the blown/stuck ride state and never writes rating. Helper recovery is deliberately unable to bypass a blown transmission.

Consequences must be explicit and escapable. Rating-gated offers say `Locked — rating` with required/current values; Kadek's priority relationship survives even while its app line is gated; the Made goal exposes the failed premium condition; one Leo text is appended idempotently. The lowest normal board tier remains available at 3.2 after repair, the ruined run pays enough to fund that repair even from a zero wallet, and focused tests must prove the post-downfall state can reach all later finale preconditions that the current packet is allowed to touch. Milestone substitution and the Ibu guarantee remain owned by RPG-20260714-09. Schema stays v11 and the map delta is none.

## 2026-07-15 - Moral-Choice Residue Feeds The Ending Without Immediate Punishment

RPG-20260714-08 establishes the Act 1 Luxury Tip Dilemma as a one-time relationship choice after the authored downfall. Villa eligibility is an explicit delivery-definition trait, not a display-name inference; only board deliveries count, and the breakdown delivery itself is excluded so the choice lands on the first later villa handoff. A pending quest flag makes the scene interruption-safe, while resolved and branch-specific collected flags make the outcome one-time and readable by the Season 1 ending without adding a save field or migration.

Both branches reuse the existing relationship-choice, hidden trust-flag, relational/algorithmic axis, and Feed-message machinery. KEEP adds Rp 500 and Platform-Efficiency/red-flag residue. RETURN adds the intended Rp 50 and Community-Trust/green-flag residue, plus one modest villa-regular ambient plant. Tip money is wallet-only: it does not alter delivery earnings, driver rating, locks, or delivery payouts. Each outcome receives one later Feed echo, and `RelationshipChoiceScene.skipOptionIndex` makes RETURN the explicit ESC/skip default while preserving the Act 0 opening's existing default. This is choice residue for later authored payoff, not an immediately surfaced karma judgment; the finale and Act 2 remain untouched.

## 2026-07-15 - Act 1's Milestone Makes Relationships A Mechanical Substitute For Ratings

RPG-20260714-09 changes move-out readiness to five completed deliveries, Rp 600 of delivery earnings, first rent paid, and either a 4.2★ driver rating or Ibu Sari's completed guarantee. The values remain centralized in the existing milestone constants and goal copy reads those constants. The guarantee is not an abstract boolean awarded by the formula: only Ibu's authored rent-day scene can write Made's recommendation-letter flag. This lets the post-downfall player progress through relationship history while preserving the untouched 4.2★ edge path for saves where the reversal never fired. It is the story's thesis expressed as gating logic, not a rating reset or hidden exception.

Move-out readiness no longer advances the act. Ibu's letter, Made's key, the move-out montage, persisted home swap, and weekly scooter contract are ordered one-time flags, and only the existing Act 2 intro card writes `currentAct = 2`. Act 2 systems therefore key from completed transition state rather than milestone math. The shared room reuses the existing Bungalow location and activity/home machinery; a resolver selects the cheap kos before the completed-move flag and the shared-room interior afterward, so v11 saves remain valid before, during, and after the swap without new schema.

## 2026-07-15 - Wave 1 Closes On One Leo Inbox Slot And Reconciled Numbers Of Record

RPG-20260714-10 treats Leo's cadence as ordered authored Feed residue, not three independent notifications. Kadek priority, the transmission breakdown, and the finale each mark one pending milestone in the existing `questFlags` map. Only the earliest pending milestone may post, and only when no unread Leo message exists. Reading the Feed releases at most one successor. Stable message IDs make each slot one-time; the breakdown reuses its Beat 3 slot rather than gaining a duplicate, and the finale body reads the existing race flags without changing the race.

Wave 1's economy numbers of record are now one tested contract: five Act 1 runs, Rp 600 delivery earnings, Kadek recurring effective terms Rp 142 versus the best normal Rp 141 at unlock, four fastest clean runs Rp 598, and five Rp 750. The Luxury Tip's Rp 500 KEEP branch is wallet-only. The reconciliation audit found the Act 0 catering definition missing its existing story-run exclusion flag; adding that single data flag makes the real run counter match the already-authored contract without changing a payout or threshold. Deterministic automation from the post-Leo boot reaches the Act 2 card in 136.74 seconds; a transparent first-read model totals 59.7 minutes inside the 45–75 minute tolerance. The model is a planning record, not human timing evidence.

## 2026-07-15 - Crews Are Scheduled Events Plus Membership Semantics

RPG-20260715-01 adds no second time system. A crew definition owns a venue anchor, weekly day/time slots, and a future regular-benefit hook; those slots generate ordinary `GameEvent` definitions consumed by the existing `EventScheduler`. The crew layer adds only invitation, membership, occurrence-deduped attendance, and regular-at-three semantics around those events.

Schema v11 is sufficient. Membership reuses canonical `world.life.joinedClubIds`; invitation, attendance count, regular activation, last-counted occurrence, and benefit-hook activation live behind typed accessors over the existing `questFlags` map. This follows the same persistence pattern as authored Act 1 state and avoids a speculative migration before real crew content exists.

The Calendar is a promise surface, not an event browser or chore list. It returns to the visible Phone strip only after Act 2 entry and contains only invited/joined crew sessions in the current week plus rent day. Joined rows are bold; invited rows are labeled without urgency. Generic events, opportunities, and goals remain out. Community stays hidden. A joined crew session may add one deduplicated open ping per occurrence; an invited-but-unjoined session adds none. Invitations never expire, and missing a session writes no penalty state.

Regular status is a structural threshold, not affinity inflation. Completing the on-site participation beat increments attendance once per occurrence. The third attendance writes regular and activates the named `regularBenefit` hook exactly once; W2-04 supplies actual effects. The temporary `act2_test_crew` and its content-neutral session existed only to prove the loop; stacked W2-02 repurposes it as Ari's canonical crew.

## 2026-07-15 - Crew Content Completes Through A Staged TALK Beat, Not A Menu Receipt

RPG-20260715-02 repurposes W2-01's temporary fixture as the single canonical `berawa_surf_run_crew`. Its Wednesday/Friday sunset circles and Sunday morning run are data variants that generate ordinary scheduled events; there is still no second clock, activity type, notification channel, or save field. The first authored beach-adjacent delivery completed after the Act 2 card calls the existing crew invitation mutation and opens an Ari portrait scene at the handoff. Invitation remains distinct from membership: the Calendar first shows the promise as invited, and choosing the first live session joins the crew before its TALK beat begins.

Attendance is awarded only when the existing dialogue continuation closes. A persisted attendance count selects a different authored member line for each of the first three visits, and a v11 quest flag makes Ari's one closed-laptop “Client calls” plant global and one-time. Sunset dialogue temporarily reuses the existing `nightQuiet` ambient bed, then returns selection to the normal phase/weather path; the clock and lighting stay authored by the existing world time. Arriving mid-window still counts because eligibility is checked on entry and completion is allowed after the line; closing or missing a window writes no penalty.

Act 2 progression must recognize both legacy social-group event IDs and W2 crew-session event IDs while the stack migrates content. Active deliveries outrank the broad Act 2 social nudge so Ari's invitation run remains physically completable. At Berawa Beach, the old Run Crew and Surf Circle join rows are hidden once Act 2 begins; their data remains load-safe for old saves, but the player-facing Act 2 venue presents Ari's combined Surf & Run Crew only. Regular status still activates only the W2-01 structural hook at attendance three; W2-04 owns any benefit effect.

## 2026-07-15 - Crew B Is Ibu's Warung Kitchen Circle, And The Squeeze Is Overheard

RPG-20260715-03 adopts the Act 2 contract's recommended Crew B: the Warung Kitchen Circle at Ibu's existing Canggu Station interior. Tuesday and Saturday evening slots generate ordinary `GameEvent` rows through the same W2-01 crew/event seam as Ari's crew. The additional semantics are invitation, membership, attendance, and regular status only; SERVE is a short staged dialogue continuation until W2-06, not a second activity system or an early Warung Rush minigame. Kadek is an off-shift ambient figure every other session and gains no choice content here.

Invitation eligibility is narrative residue over existing mutations. Each completed Act 2 Ibu-board delivery increments a v11 quest-flag counter, and a successful Act 2 rent payment writes an alternative boolean gate. Neither path auto-invites: the player must approach Ibu for her proud summons, which leaves the crew invited but not joined and therefore lights the existing Calendar promise surface without a session-open ping.

The platform squeeze is authored as something the player overhears during whichever Kitchen session they actually attend first. Preparing that participation beat writes one persisted squeeze flag and day; completing the continuation uses the core attendance mutation. Ibu never explains the 30% commission to the player. One later approach consumes a single deflection line, and two days later one stable Feed message reports only a Rp 2 chalk-board increase. These flags are narrative and ending-readable, not a commission percentage, price model, payout modifier, or warung economy simulation. Schema remains v11, invitations remain non-expiring, missed sessions remain consequence-free, and W2-04 still owns real regular benefits.

## 2026-07-15 - Relationship Milestones Unlock Structure, Not Character Power

RPG-20260715-04 fills existing crew `regularBenefit` hooks and the existing `friendly` affinity tier with access, dialogue, and price changes. Crew/relationship progress never creates a multiplier, XP curve, new meter, or parallel perk registry. Surf recovery is one flat additive adjustment on the authored Sunday participation result through the canonical meter machinery; Kitchen status changes one existing meal offer and one existing interior-access window; affinity changes authored copy, row priority, or availability of an existing timed effect.

The economy numbers are narrow and explicit. Nasi Bungkus retains its Rp 35 base and costs Rp 30 only for Kitchen regulars. Kadek's existing Focus Buffer pastry costs Rp 18 at BAKED., requires his existing priority flag plus the existing friendly tier, is purchasable once per in-game day, and activates the same 180-minute flag used in Act 1. No Wave-1 earning, rent, rating, payout, tip, repair, or finale number changes.

Unlock visibility is a read model, not duplicate progression state. Five stable story-message IDs make Feed announcements one-time, while Profile derives its five rows from canonical crew, affinity, and priority state on every render. The one daily pastry marker fits the established v11 quest-flag pattern, so no schema bump is justified. Act 0/1 warung access stays unchanged; Act 2 active-delivery access remains fail-forward; missed sessions and invitations retain zero-FOMO semantics.

## 2026-07-15 - The PDA Reveals Existing Independent Axes And Does Not Pretend They Are A Seesaw

RPG-20260715-05 confirms that the engine's reputation fields are already live but are not strict opposites. `rootedAxis` records rooted versus extractive residue; `relationalAxis` records relational versus algorithmic residue. Existing choices can move one axis or both, so no truthful implementation can claim that maximizing one necessarily lowers the other. W2-05 changes no choice math. Its permanent Profile copy instead distinguishes “what the app rewards” from “what the street remembers” and explicitly says a choice may move one, both, or neither.

The phone's `0..100` numbers are presentation-only projections over the canonical `-100..100` fields: Community Trust / leaked `metric_x` reads `rootedAxis`, while Platform Efficiency reads the inverse presentation direction of `relationalAxis`. Neither projection is saved or tracked as a new metric. History markers likewise derive only from existing opportunity, pickup, delivery, and quest flags; missing branch evidence produces no row.

Discovery is a one-time diegetic Profile moment gated by actual Wednesday/Friday sunset attendance plus the persisted overheard squeeze. One stable Feed message announces it, and one existing-pattern v11 quest flag remembers that the surface has been seen. Profile owns the permanent section; the public phone gets no new tab, Community remains hidden, and neither opening nor completing the reveal writes reputation.

## 2026-07-15 - SERVE Is Ibu's Dinner-Rush Verb, Not A Floating Venue Activity

RPG-20260715-06 removes Warung Rush from the generic activity registry and anchors its only new launch paths at Ibu's: the participation beat of an ordinary scheduled Kitchen Circle event, and one member-only Thursday busy-night Feed ping per week. Neither path creates a scheduler, minigame mode, notification channel, or alternate warung economy. The internal `warung_lunch_rush` history/save ID remains solely for v11 compatibility; a context-free in-progress legacy round may finish, but no surface can start another.

Kitchen attendance belongs to completion of the real rush, not its framing dialogue. Resolution uses the existing event participation, intent, occurrence-dedupe, and crew regular-at-three machinery. A zero-score room counts and missing a room writes nothing. Strong session performance adds copy from a three-line rotating crew pool; busy-night play never increments attendance. The existing 75-second round, patience, order cadence, maximum concurrent orders, difficulty step, counter/table interaction, score math, keyboard input, and touch input are unchanged.

Performance residue is deliberately modest and one-way: score-clamped tips run Rp 12–28 and Ibu affinity runs +1–3, with weak play explicitly teased rather than docked. Busy-night meters use the existing flat adjustment path and no result can reduce money or affinity. After the squeeze, Ibu's line about each self-served plate escaping the app's tax gives the existing mechanic narrative weight without adding commission simulation. Wave-1 delivery/rent/rating reconciliation remains untouched; schema stays v11 and the map delta is none.

## 2026-07-15 - Kadek's Choice Leaves Ending-Readable Residue, Not A Punishment Branch

RPG-20260715-07 stages the Whistleblower Sourdough as one authored priority delivery, not a random wrong-address rule. The row exists only after Act 2 entry, Kadek's existing friendly tier, and Kitchen Circle regular status; while unresolved it replaces his ordinary recurring priority delivery and becomes acceptable after 20:00. The pickup uses an existing route, and the return uses BAKED.'s existing interior. Evidence, pending choice, resolution, branch, and closure all fit the established v11 quest-flag pattern, so no schema or map change is justified.

The residue map is exact and branch-neutral outside the story axis. PROTECT is the explicit ESC/skip default, adds `+5` to the existing rooted/Community-Trust axis, leaves the relational/Platform-Efficiency axis unchanged, and plants Kadek baking under his own name. EXPOSE is framed as honesty about the squeeze economy, adds `-3` to Community Trust, leaves Platform Efficiency unchanged, and plants the circle baking together. Both preserve Kadek affinity, delivery terms, rating, meters, crew state, prices, and access. Their branch flags and summaries are intended for the Wave 3 ending; no new metric or immediate moral verdict is introduced.

Moonlighting ends through Kadek's next actually attended Kitchen session, not at choice click and not on a missed calendar window. One pending flag makes Kadek present for that occurrence, one branch-specific line states that he ended the outside work himself, and consumption records the ending once. PROTECT resolves privately; EXPOSE lets Ibu and the circle close around him without destroying the relationship. The brother, debt, and corporate café remain spoken context rather than simulated systems or new NPCs.

## 2026-07-15 - Act 2 Completes At The Circle; Wave 3 Owns The Cut

RPG-20260715-08 keeps Julian Vance narrative-only. Completing the existing PDA reveal schedules one authored Milk & Madu relationship scene in his established habitat. DECLINE and TAKE CARD are mutually exclusive v11 quest-flag residue and change no money, rating, reputation, affinity, meters, work, or access. TAKE CARD alone can append one stable passive-aggressive Feed message after 24 in-game hours. It is not a job branch, Enclave system, buyout, or lockout.

The sunset-seat foundation is both crew regular flags plus the overheard squeeze and completed PDA reveal. The playable action also waits for Vance's preceding scene to resolve so Beat 7 cannot be silently skipped; either Vance response satisfies that sequencing condition. Sunday 17:00–20:00 is only an availability window. Missing it writes no state and the action returns next week.

The finale is one persisted staged scene over the existing Berawa Beach, authored clock, cutscene, dialogue, and world-scene paths. Branch variants are projections over real tip, sourdough, and No-Questions flags/opportunity history; absent evidence yields no invented line. Leo is never spawned. His absence is represented by Ari's single surge line. The open seat, not a reward screen, is the Act 2 payoff.

All three toast choices are tone selection only. They write the same `act2:finale:complete` boundary plus the selected flavor, save immediately, and do not change any mechanic. This flag replaces the old four-goal scaffold as the authoritative Act 2 completion gate for future readiness. `currentAct` remains `2`, and no `ACT 2 — COMPLETE` card is produced. Wave 3 begins from the circle's completed state and owns the Season 1 END card, ending sequence, and any later act transition. Schema remains v11 and the map delta is none.
