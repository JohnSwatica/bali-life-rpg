...
Immediate next move (CEO directive 2026-07-14, second founder feedback): **Wave 1.5 — interface & world overhaul** (PLAYTEST_02.md verbatim; contract `docs/INTERFACE_WORLD_OVERHAUL_2026-07-14.md`). Wave 1 story is PAUSED after Beats 1–2 (both merged: Kadek PR #5, Made PR #7 at `928ae8d`; dev proof harness PR #6). Workstream A (`RPG-20260714-05`, phone diet) is MERGED (`be28ffd`, PR #8): 14 filler templates culled via generation allowlist, feed reordered story→jobs, tabs 10→4. Remaining Workstreams B+C+D (venue purpose pass, street density, interior identity) are bundled into ONE packet by CEO instruction — `RPG-20260714-06` (issued, + two -05 review riders). Then the Wave 1.5 screenshot gate, then Beats 3–6 resume (breakdown reversal next). Wave 1 close moves to ~Jul 22–24; Aug 5–12 launch window stands. Hard boundaries unchanged: no multiplayer, backend, AI calls, real commerce, Google data, or the Act 3 management sim (Act 3+ = Season 2).
