...
Immediate next move (CEO directive 2026-07-14): **the Season 1 launch program — `LAUNCH_PLAN_SEASON1_2026-07-14.md` is the program of record.** Scope: Acts 0–2 shipped as "Season 1" on itch.io + the existing Pages URL; target window Aug 5–12, 2026 at 2–3 packets/day. Testing gates are retired by CEO authority — no alpha/founder-playtest gates, no outsider-share prerequisite; quality is proven per-packet (tests/build/smoke/proof, Claude reviews and merges per Standing Rule 9) and per-wave (Claude's full AI playthrough scored against the benchmark rubric). RPG-20260713-04 merged to `main` 2026-07-14 (`7283909`, PR #3) — Wave 0 remaining: the wave-gate AI playthrough of the full first ten minutes (including a reading-time-modeled pacing estimate against the 9–11 min target, since the deterministic harness fast path of 3:32 can't validate it) and one polish packet from its misses. Then Wave 1 (Act 1 backbone: Kadek's beat, Made's offer, midpoint breakdown, luxury tip dilemma, move-out finale). Hard boundaries unchanged: no multiplayer, backend, AI calls, real commerce, Google data, or the Act 3 management sim (Act 3+ = Season 2).
