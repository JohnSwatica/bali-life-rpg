...
Immediate next move (launch program, 2026-07-14): **Wave 1 — Act 1 backbone.** Wave 0 is CLOSED: gate review passed conditionally, RPG-20260714-01 (presentation polish) reviewed and merged (`8721a64`, PR #4; one accepted nit — player sprite over card text — carried as a rider). Wave 1 contract: `docs/ACT1_BACKBONE_2026-07-14.md` (Kadek TP1 → Made TP2 → breakdown reversal → tip dilemma → Ibu-vouch finale with the milestone adjustment → Leo glue). Beat 1 (`RPG-20260714-02`, Kadek's priority list + z-order rider) is MERGED (`65b0de6`, PR #5; smoke villa-leg timeout triaged as pre-existing harness flake — passed on review re-run). Beat 2 (`RPG-20260714-03`, Made's room offer) is feature-complete on `feat/rpg-20260714-03-made-room-offer` (286 tests + build green) but its headless proof stalled on harness economics — the per-packet full-smoke DoD standard was wrong and is retired (see the proof standard in `docs/ACT1_BACKBONE_2026-07-14.md`). `RPG-20260714-04` (dev proof harness: authored boot states via gameplay mutations + stable interaction API) is issued and runs NEXT; -03 finalizes its proof cheaply after -04 merges. Then Beat 3 (breakdown reversal) as `RPG-20260714-05`. Wave 1 gate: full AI playthrough Act 0 → Act 2 card before Wave 2. Target close Jul 20–21. Hard boundaries unchanged: no multiplayer, backend, AI calls, real commerce, Google data, or the Act 3 management sim (Act 3+ = Season 2).
