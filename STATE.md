...
- HUD/minimap viewport clipping fix: the six right-side HUD buttons and minimap now render on an unzoomed UI camera, while the world camera keeps the authored-street zoom. The Phaser Scale Manager no longer uses device-pixel-ratio as canvas zoom, which was the source of the off-screen/cropped presentation regression.
...
- Phaser now uses `RESIZE` mode without canvas zoom so the canvas matches the live viewport. Antialiasing remains enabled; the authored tile map and UI are kept crisp by avoiding browser-level canvas scaling.
