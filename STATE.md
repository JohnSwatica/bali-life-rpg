...
Immediate next move (launch program, 2026-07-14): **Wave 1 — Act 1 backbone.** Wave 0 is CLOSED: gate review passed conditionally, RPG-20260714-01 (presentation polish) reviewed and merged (`8721a64`, PR #4; one accepted nit — player sprite over card text — carried as a rider). Wave 1 contract: `docs/ACT1_BACKBONE_2026-07-14.md` (Kadek TP1 → Made TP2 → breakdown reversal → tip dilemma → Ibu-vouch finale with the milestone adjustment → Leo glue). `RPG-20260714-02` (Kadek's priority list + z-order rider) is issued — packets run sequentially, one at a time. Wave 1 gate: full AI playthrough Act 0 → Act 2 card before Wave 2. Target close Jul 20–21. Hard boundaries unchanged: no multiplayer, backend, AI calls, real commerce, Google data, or the Act 3 management sim (Act 3+ = Season 2).
