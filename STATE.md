...
- **RPG-20260714-01 complete locally on `main`:** Wave 0's six presentation blockers are addressed without gameplay/state changes: all act cards now use a centered text scrim; letterbox-aware HUD placement and story-phone deposit placement keep the Rp 560 target visible; ambient toasts queue through cutscenes and authored phone beats; NusaDrop leaderboard rows carry delivery counts; the kos resolve stages a doorway landlord silhouette; and delivery celebration text holds with high contrast at night. Schema remains v11. Verification: 43 files / 278 tests, production build, and a fresh full `npm run smoke` pass with before/after crop proof in `docs/RPG-20260714-01_PRESENTATION_POLISH_PROOF.md`.
...
Immediate next move (launch program, 2026-07-14): **Wave 1 — Act 1 backbone.** Wave 0 is CLOSED: gate review passed conditionally, RPG-20260714-01 (presentation polish) reviewed and merged (`8721a64`, PR #4; one accepted nit — player sprite over card text — carried as a rider). Wave 1 contract: `docs/ACT1_BACKBONE_2026-07-14.md` (Kadek TP1 → Made TP2 → breakdown reversal → tip dilemma → Ibu-vouch finale with the milestone adjustment → Leo glue). Beat 1 (`RPG-20260714-02`, Kadek's priority list + z-order rider) is MERGED (`65b0de6`, PR #5; smoke villa-leg timeout triaged as pre-existing harness flake — passed on review re-run). Beat 2 (`RPG-20260714-03`, Made's room offer) is feature-complete on `feat/rpg-20260714-03-made-room-offer` (286 tests + build green) but its headless proof stalled on harness economics — the per-packet full-smoke DoD standard was wrong and is retired (see the proof standard in `docs/ACT1_BACKBONE_2026-07-14.md`). `RPG-20260714-04` (dev proof harness) is MERGED (`e345b0a`, PR #6): three gameplay-constructed boot states, dev interaction API, `npm run beat-proof` — Kadek beat proof reproduced in 27.6s vs ~4 min before; dev hooks verified absent from the production bundle. NEXT: -03 finalizes on its branch using the harness (amended DoD in its packet file), then Beat 3 (breakdown reversal) as `RPG-20260714-05`. Wave 1 gate: full AI playthrough Act 0 → Act 2 card before Wave 2. Target close Jul 20–21. Hard boundaries unchanged: no multiplayer, backend, AI calls, real commerce, Google data, or the Act 3 management sim (Act 3+ = Season 2).
