...
Immediate next move (CEO directive 2026-07-14, second founder feedback): **Wave 1.5 — interface & world overhaul**, then story resumes. Full-horizon roadmap + complete draft set through launch produced 2026-07-14 by CEO order: **`ROADMAP_2026-07-14.md` is the narrative map** (today's ledger → week plan → ending goal); 20 draft packets live in `docs/prompts/drafts/` (Wave 1 Beats 3–6, Wave 2 W2-01…08 on `docs/ACT2_FINDING_YOUR_PEOPLE_2026-07-14.md`, Wave 3 on `docs/SEASON1_ENDING_2026-07-14.md`, Wave 4 W4-01…06 + `docs/ITCH_PAGE_DRAFT_2026-07-14.md`). DRAFTS ARE NOT FERRYABLE — each gets corrected, re-ID'd (RPG-YYYYMMDD-NN), and SHA-pinned at issue time. Current sequence: RPG-20260714-06 (in flight at Codex: venue purpose + street density + interior identity, bundled) → my Wave 1.5 screenshot gate → issue Beat 3 from its draft. Workstream A (`RPG-20260714-05`, phone diet) is MERGED (`be28ffd`, PR #8). Hard boundaries unchanged: no multiplayer, backend, AI calls, real commerce, Google data, or the Act 3 management sim (Act 3+ = Season 2).
